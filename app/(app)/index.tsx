...
import { Camera, Map, ChartLine as LineChart, Leaf, Sun, Droplets, AlertTriangle, LogOut, MapPin, MapPinOff } from 'lucide-react-native';
...
const CROP_ORBS = [
  ...
  },
  {
    ...
    x: -42, y: 590, size: 130, dur: 4600, delay: 2500, fy: 13, fx: 7,
  },
  {
    name: 'Berries',
    uri: 'https://images.unsplash.com/photo-1601004890684-d8cbf643f5f2?w=400&q=85',
    color: '#6d28d9', glow: 'rgba(168,85,247,0.28)',
    x: width - 106, y: 635, size: 128, dur: 5000, delay: 3200, fy: -11, fx: -4,
  },
];

const FloatingOrb = ({ orb }: { orb: typeof CROP_ORBS[0] }) => {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={s.bgLayer} pointerEvents="none">
        {CROP_ORBS.map((orb) => <FloatingOrb key={orb.name} orb={orb} />)}
      </View>

      <View style={s.centerVignette} pointerEvents="none" />

      <TouchableOpacity style={s.logoutBtn} onPress={() => signOut(auth).then(() => router.replace('/(auth)'))}>
        ...
        <View style={s.header}>
          ...
          {user && (
            <Reanimated.View entering={FadeInDown.delay(200).duration(700)}>
              <View style={[s.welcomeCard, { borderColor: colors.welcomeBorder }]}>
                <View style={s.welcomeText}>
                  <Text style={s.greetLabel}>BIENVENIDO</Text>
                  <Text style={[s.userName, { color: colors.textPrimary }]}>{user.displayName || user.email}</Text>
                </View>
                <Avatar user={user} />
              </View>
            </Reanimated.View>
          ...
          <Reanimated.View entering={FadeInDown.delay(260).duration(700)}>
            <View style={s.locationToggleCard}>
              <View style={s.locationToggleLeft}>
                ...
                <View style={{ marginLeft: 10 }}>
                  <Text style={[s.locationToggleTitle, { color: locationEnabled ? '#0f766e' : '#64748b' }]}>
                    Ubicación
                  </Text>
          ...
          {locationEnabled && municipio.length > 0 && (
            <Reanimated.View entering={FadeInDown.delay(300).duration(700)} style={s.locRow}>
              <View style={[s.locBadge, { backgroundColor: colors.cardBg }]}>
                <Text style={[s.locText, { color: colors.textPrimary }]}>📍 {municipio}</Text>
              </View>
              <View style={[s.timeBadge, { backgroundColor: colors.cardBg }]}>
                <Text style={[s.timeText, { color: colors.textPrimary }]}>{dateTime.time}</Text>
                <Text style={[s.dateText, { color: colors.textSecondary }]}>{dateTime.date}</Text>
              </View>
            </Reanimated.View>
          )}
          ...
          <Reanimated.View entering={FadeInDown.delay(420).duration(700)} style={s.stats}>
            <View style={s.statItem}>
              ...
              <Text style={s.statLbl}>Temperatura</Text>
            </View>
            <View style={s.statDiv} />
            <View style={s.statItem}>
              <Droplets color="#38bdf8" size={22} />
              <Text style={[s.statVal, { color: colors.textPrimary }]}>{humidity !== null ? `${humidity}%` : '—'}</Text>
              <Text style={s.statLbl}>Humedad</Text>
            </View>
            <View style={s.statDiv} />
            <View style={s.statItem}>
              <Leaf color={colors.accent} size={22} />
              <Text style={[s.statVal, { color: colors.textPrimary }]}>{healthPct !== null ? `${healthPct.toFixed(0)}%` : 'N/A'}</Text>
              <Text style={s.statLbl}>Salud</Text>
            </View>
          </Reanimated.View>
        </View>
        ...
        <Reanimated.View entering={FadeInUp.delay(500).duration(700)} style={{ marginTop: -5, marginBottom: 12 }}>
          {!locationEnabled
            ...
            : (!location || !location.coords)
              ? <View style={s.mapPh}><Text style={{ color: '#94a3b8', fontSize: 14 }}>📍 Obteniendo ubicación…</Text></View>
              : <MapViewComponent location={location} errorMsg={errorMsg} />
          }
        </Reanimated.View>

        <Reanimated.View entering={FadeInUp.delay(620).duration(700)} style={s.grid}>
          <TouchableOpacity style={s.mainCard} onPress={() => router.push('/scan')}>
            <LinearGradient colors={['#6ee7b7', '#059669']} style={s.cardGrad}>
              <View style={s.cardIcon}><Camera color="#fff" size={26} /></View>
              <Text style={s.cardTitle}>Escanear</Text>
              <Text style={s.cardSub}>Detecta enfermedades{'\n'}en tus cultivos</Text>
            </LinearGradient>
          </TouchableOpacity>

          <View style={s.secCards}>
            <TouchableOpacity style={[s.secCard, { marginBottom: 12 }]} onPress={() => router.push('/mapping')}>
              <LinearGradient colors={['#fcd34d', '#f59e0b']} style={s.secGrad}>
                <View style={s.secIcon}><Map color="#fff" size={20} /></View>
                <Text style={s.secTitle}>Mapeo</Text>
              </LinearGradient>
            </TouchableOpacity>
            <TouchableOpacity style={[s.secCard, { marginBottom: 12 }]} onPress={() => router.push('/results')}>
              <LinearGradient colors={['#67e8f9', '#0891b2']} style={s.secGrad}>
                <View style={s.secIcon}><LineChart color="#fff" size={20} /></View>
                <Text style={s.secTitle}>Resultados</Text>
              </LinearGradient>
            </TouchableOpacity>
            <TouchableOpacity style={s.secCard} onPress={() => router.push('/agenda')}>
              <LinearGradient colors={['#86efac', '#16a34a']} style={s.secGrad}>
                <View style={s.secIcon}><AlertTriangle color="#fff" size={20} /></View>
                <Text style={s.secTitle}>Medidas</Text>
              </LinearGradient>
            </TouchableOpacity>
          </View>
        </Reanimated.View>
...
const s = StyleSheet.create({
  ...
  logoutBtn: {
    position: 'absolute', top: 30, right: 16, zIndex: 20,
  ...
  header: { paddingTop: 74, paddingHorizontal: 22, marginBottom: 18 },
